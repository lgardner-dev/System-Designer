            ...
            layer.sort_by(|&a, &b| {
                let score = |i: usize| {
                    ...
                    if neighbours.is_empty() {
                        ...
                        neighbours.iter().map(|n| order[n]).sum::<f32>() / neighbours.len() as f32
                    }
                };
...
fn arranged(p: &Project, sid: &str, axis: Axis) -> Result<BTreeMap<String, Position>> {
...
pub(super) fn controls(app: &mut Designer, ui: &mut egui::Ui) {
    let id = egui::Id::new("system-designer.arrangement-axis");
    let mut axis = ui
        .ctx()
        .data(|d| d.get_temp::<Axis>(id))
        .unwrap_or_default();
    let mut run = false;
    ui.horizontal_wrapped(|ui|{
        ui.strong("C · Connection layout");
        ui.selectable_value(&mut axis,Axis::Horizontal,"Left to right");
        ui.selectable_value(&mut axis,Axis::Vertical,"Top to bottom");
        run=ui.button("Arrange by connections").on_hover_text("Arrange this level only. Feedback groups stay cyclic; the rest uses directional layers. One undoable layout edit; no edge or contract changes.").clicked();
    });
    ui.small("Cycles use local rings, not invented start/end points. Layout direction is not execution order. Use Undo to restore your positions.");
    ui.ctx().data_mut(|d| d.insert_temp(id, axis));
    if run {
        app.canvas.cancel();
        let sid = app.current.clone();
        let candidate = arranged(app.store.project(), &sid, axis).and_then(|positions| {
            edit::candidate(app.store.project(), |p| {
                p.layout.insert(sid, positions);
                Ok(())
            })
        });
        app.publish("Arrange by connections", candidate);
